...
class fm_module_buildconf extends fm_shared_module_buildconf {
	...
	function buildServerConfig($post_data) {
		...
		if ($fmdb->num_rows) {
			...
			// error_log(print_r($config_dir_result, TRUE)); 
			...
			if ($keys) {
				...
				$data->files[dirname($server_config_file) . '/' . $server_application_name . '.conf.keys'] = array('contents' => $key_config, 'mode' => 0400);
			
				//$config .= "include \"" . dirname($server_config_file) . "/named.conf.keys\";\n\n";
				$config .= "include \"" . dirname($server_config_file) . "/" . $server_application_name . ".conf.keys\";\n\n";
			}
			$config .= $servers;
			...
			if (isDebianSystem($server_os_distro)) {
				...
				$data->files[dirname($server_config_file) . '/' . $server_application_name . '.conf.options'] = array('contents' => $config, 'mode' => 0444, 'chown' => 'root');
				//$config = $zones . "include \"" . dirname($server_config_file) . "/named.conf.options\";\n\n";
				$config = $zones . "include \"" . dirname($server_config_file) . "/" . $server_application_name . ".conf.options\";\n\n";
			...
			/** fDNS.conf.fstack */
			if (!strcmp("fDNS",$server_application_name)) {
				// $myFile = "testFile.txt";
				...
				// fwrite($fh, $stringData);
				// fclose($fh);
				//$config = "fstack {\n    file \"" . dirname($server_config_file) . "/f_stack.conf\";\n    pci \"0000:00:04.0\";\n    mem \"mem0\";\n}";
				//$data->files[dirname($server_config_file) . '/' . $server_application_name . '.conf.fstack'] = array('contents' => $config, 'mode' => 0444, 'chown' => 'root');
				//$config = $zones . "include \"" . dirname($server_config_file) . "/named.conf.options\";\n\n";
				$config = $data->files[$server_config_file]['contents'] . "include \"" . dirname($server_config_file) . "/" . $server_application_name . ".conf.fstack\";\n\n";
				$data->files[$server_config_file] = array('contents' => $config, 'mode' => 0444, 'chown' => 'root');
				$config = $zones;
			}
			...
			if (isDebianSystem($server_os_distro)) {
				...
				$data->files[dirname($server_config_file) . '/' . $server_application_name . '.conf.local'] = array('contents' => $config, 'mode' => 0444, 'chown' => 'root');
				//$config = $data->files[$server_config_file]['contents'] . "include \"" . dirname($server_config_file) . "/named.conf.local\";\n\n";
				$config = $data->files[$server_config_file]['contents'] . "include \"" . dirname($server_config_file) . "/" . $server_application_name .  ".conf.local\";\n\n";
			}
			...
			//error_log(print_r(get_object_vars($data), TRUE)); 
			return array(get_object_vars($data), $message);
		}
